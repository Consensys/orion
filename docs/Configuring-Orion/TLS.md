...
## Populating tlsknownclients and tlsknownservers File

Use one of the TLS trust modes other than `whitelist` until the `tlsknownclients` and `tlsknownservers`
files are populated. When the files are populated, restart the nodes with the TLS trust mode set to 
`whitelist` for `tlsclienttrust` and `tlsservertrust`. 
