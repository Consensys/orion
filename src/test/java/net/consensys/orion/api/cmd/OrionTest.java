...
public class OrionTest {
  ...
  public void createCertificateIfFilesAreNotThereDefault() throws Exception {
    ...
    assertTrue(workDir.resolve(config.tlsClientKey()).toFile().exists());
    assertTrue(workDir.resolve(config.tlsClientCert()).toFile().exists());
    assertTrue(workDir.resolve(config.tlsServerKey()).toFile().exists());
    assertTrue(workDir.resolve(config.tlsServerCert()).toFile().exists());
    assertTrue(workDir.resolve(config.tlsKnownClients()).toFile().exists());
    assertTrue(workDir.resolve(config.tlsKnownServers()).toFile().exists());
  }
  ...
  public void createCertificateIfFilesAreNotThereAbsolutePath() throws Exception {
    ...
    assertTrue(workDir.resolve(config.tlsKnownServers()).toFile().exists());
    assertTrue(
        workDir.resolve(config.tlsClientKey()).toString(),
  ...
  public void autoGeneratedCertsAreValid() throws Exception {
    ...
    ((MemoryConfig) config).setWorkDir(workDir);
    Orion.generateCertificatesAndMissingFiles(config);
